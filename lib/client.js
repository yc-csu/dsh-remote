...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RemoteWorkspacePage() {
      ...
      const [guard, setGuard] = React.useState('high')

      const refreshGuard = () => api('GET', '/dsh-remote/guard').then((r) => { if (r && (r.mode === 'high' || r.mode === 'low')) setGuard(r.mode) }).catch(() => {})
      const setGuardMode = (mode) => {
        setBusy(true)
        api('POST', '/dsh-remote/guard', { mode })
          .then((r) => { setGuard(r.mode); setMsg('已切换为' + (r.mode === 'low' ? '低自主性' : '高自主性') + '模式') })
      ...
      React.useEffect(() => { refresh(); refreshGuard() }, [])
      ...
      return React.createElement('div', { style: { padding: 16, display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 760 } },
        ...
        React.createElement('div', { style: boxS },
          React.createElement('div', { style: { marginBottom: 6, fontSize: 13, fontWeight: 600 } }, '远程操作自主性'),
          React.createElement('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
            React.createElement('button', {
              style: { ...buttonS, fontWeight: guard === 'high' ? 700 : 400, borderColor: guard === 'high' ? T.ok : T.border },
              onClick: () => setGuardMode('high'), disabled: busy,
            }, '高自主性'),
            React.createElement('button', {
              style: { ...buttonS, fontWeight: guard === 'low' ? 700 : 400, borderColor: guard === 'low' ? T.ok : T.border },
              onClick: () => setGuardMode('low'), disabled: busy,
            }, '低自主性'),
            React.createElement('span', { style: { fontSize: 11, opacity: 0.7 } },
              guard === 'low'
                ? '低自主性：AI 操作服务器前会先列行动清单，创建/执行/删除/修改都需你批准。'
                : '高自主性：AI 自由读写执行，但删除/修改文件仍需你批准。'),
          ),
        ),
        React.createElement('div', { style: boxS },
...
// appending that directory as the new deepest level (path is pieced together).
    ...
    // /dsh-remote/assets/* route (no CDN, no network dependency).
    ...
    function ensureXterm() {
      return Promise.all([
        loadAsset('/dsh-remote/assets/xterm.css', 'css'),
        loadAsset('/dsh-remote/assets/xterm.js', 'js'),
        loadAsset('/dsh-remote/assets/addon-fit.js', 'js'),
      ]).then(() => {
    ...
    // ── Floating window hook: drag + resize + minimize ──────────────────────
    // Returns { style, onPointerDown, onResizeDown, minimized, toggleMinimize,
    //           setMinimized }. onPointerDown goes on the title bar; onResizeDown
    // goes on a bottom-right grip. Drag and resize clamp to the viewport.
    function useWindow(initialSize) {
      const [pos, setPos] = React.useState({ x: 0, y: 0 })
      const [size, setSize] = React.useState(initialSize || { w: 920, h: 640 })
      ...
      const onPointerDown = (e) => {
        ...
        const move = (ev) => {
          ...
          nx = Math.max(-w * 0.5, Math.min(nx, w * 0.5))
          ny = Math.max(-h * 0.5, Math.min(ny, h * 0.5))
          setPos({ x: nx, y: ny })
        }
        const up = () => {
      ...
      const onResizeDown = (e) => {
        ...
        const move = (ev) => {
          ...
          nw = Math.max(360, Math.min(nw, window.innerWidth * 0.98))
          nh = Math.max(260, Math.min(nh, window.innerHeight * 0.96))
          setSize({ w: nw, h: nh })
        }
        const up = () => {
          window.removeEventListener('pointermove', move)
      ...
      return { style, onPointerDown, onResizeDown, minimized, toggleMinimize, setMinimized }
    }
    ...
    function RemoteTerminal({ onClose }) {
      ...
      const win = useWindow({ w: 920, h: 640 })
      ...
      React.useEffect(() => {
        ...
        const boot = async () => {
          ...
          try {
            ...
            ws = new WebSocket(proto + '//' + location.host + '/dsh-remote/term')
            wsRef.current = ws
      ...
      const viewImage = (p) => {
        ...
        const url = '/dsh-remote/img?path=' + encodeURIComponent(path)
      ...
      // When the window is resized, re-fit the terminal.
      React.useEffect(() => {
        try { if (fitRef.current) fitRef.current.fit() } catch {}
      }, [win.minimized])

      return React.createElement('div', { style: termStyles.outer, onClick: (e) => { if (e.target === e.currentTarget) onClose() } },
        React.createElement('div', { style: { ...termStyles.panel, ...win.style, height: win.minimized ? 'auto' : win.style.height }, onClick: (e) => e.stopPropagation() },
          React.createElement('div', { style: { ...termStyles.head, cursor: 'move', userSelect: 'none' }, onPointerDown: win.onPointerDown },
            React.createElement('div', { style: termStyles.title }, '远程终端（dsh-remote）'),
            ...
            React.createElement('button', { style: buttonS, onClick: win.toggleMinimize, title: win.minimized ? '还原' : '最小化' }, win.minimized ? '▢ 还原' : '— 最小化'),
          ...
          win.minimized
            ...
            : React.createElement(React.Fragment, null,
                ...
                React.createElement('div', {
                  onPointerDown: win.onResizeDown,
    ...
    // ── File manager (WinSCP-style: remote browser + upload/download) ─────
    const fmStyles = {
      overlay: { position: 'fixed', inset: 0, zIndex: 9998, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 14 },
      panel: { background: v('--dsw-alias-bg-layer-1', '#111114'), border: '1px solid ' + T.borderStrong, borderRadius: 12, boxShadow: '0 12px 48px rgba(0,0,0,0.5)', width: 'min(860px, 96vw)', height: 'min(600px, 88vh)', display: 'flex', flexDirection: 'column', overflow: 'hidden' },
      head: { display: 'flex', gap: 8, alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid ' + T.border, flexShrink: 0, cursor: 'move', userSelect: 'none' },
      pathBar: { display: 'flex', gap: 6, alignItems: 'center', padding: '8px 14px', borderBottom: '1px solid ' + T.border, flexShrink: 0 },
      table: { flex: 1, minHeight: 0, overflowY: 'auto', fontSize: 12.5 },
      row: { display: 'flex', gap: 8, alignItems: 'center', padding: '6px 14px', borderBottom: '1px solid ' + T.border, cursor: 'default' },
      rowHover: { display: 'flex', gap: 8, alignItems: 'center', padding: '6px 14px', borderBottom: '1px solid ' + T.border, background: T.hoverBg, cursor: 'default' },
      name: { flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace' },
      size: { width: 90, textAlign: 'right', opacity: 0.75, fontFamily: 'monospace', whiteSpace: 'nowrap' },
      mtime: { width: 150, textAlign: 'right', opacity: 0.65, fontFamily: 'monospace', whiteSpace: 'nowrap', fontSize: 11 },
    ...
    function FileManager({ onClose }) {
      ...
      const [loading, setLoading] = React.useState(false)
      const [err, setErr] = React.useState('')
      const [sel, setSel] = React.useState(null) // selected item name
      const [info, setInfo] = React.useState(null)
      const [busy, setBusy] = React.useState(false)
      const [msg, setMsg] = React.useState('')
      ...
      const win = useWindow({ w: 860, h: 600 })

      const loadDir = (p) => {
        setLoading(true); setErr(''); setSel(null)
        api('GET', '/dsh-remote/list?path=' + encodeURIComponent(p || '/'))
          .then((r) => {
            setPath(r.path || p)
            setItems(r.items || [])
          })
      ...
      // Default to the connected user's REAL home dir (query the host, since it
      // may be /public/home/<user> rather than /home/<user>), so the file
      // manager opens somewhere familiar instead of an empty/missing path.
      React.useEffect(() => {
        api('GET', '/dsh-remote/machines').then((r) => {
          const cur = (r.machines || []).find((m) => m.id === r.currentId)
          setInfo(cur || null)
        }).catch(() => {})
        api('GET', '/dsh-remote/home').then((r) => {
          if (r && r.ok && r.home) loadDir(r.home)
          else loadDir('/')
        }).catch(() => { loadDir('/') })
      }, [])
      ...
      const enter = (name) => loadDir(join(path, name))

      const download = (name) => {
        const url = '/dsh-remote/download?path=' + encodeURIComponent(join(path, name))
      ...
      const upload = (file) => {
        if (!file) return
        setBusy(true); setMsg('上传中：' + file.name + ' …')
        file.arrayBuffer().then((buf) => {
          return fetch('/dsh-remote/upload?dir=' + encodeURIComponent(path) + '&name=' + encodeURIComponent(file.name), {
            method: 'POST',
            headers: { 'Content-Type': 'application/octet-stream' },
            body: buf,
          }).then((r) => r.json())
        }).then((r) => {
          if (r && r.ok) { setMsg('上传完成：' + file.name); loadDir(path) }
          else setErr((r && r.error) || '上传失败')
        }).catch((e) => setErr(String((e && e.message) || e))).finally(() => setBusy(false))
      }

      const doDelete = (name) => {
        if (!window.confirm('确定删除 ' + name + ' ？（不可恢复）')) return
        setBusy(true)
        api('POST', '/dsh-remote/fs', { action: 'delete', path: join(path, name) })
          .then(() => { setMsg('已删除：' + name); setSel(null); loadDir(path) })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setBusy(false))
      }
      ...
      const doMkdir = () => {
        ...
        if (!n) return
        setBusy(true)
        api('POST', '/dsh-remote/fs', { action: 'mkdir', path: join(path, n) })
          .then(() => { setMsg('已创建：' + n); setMkdirOpen(false); setMkdirName(''); loadDir(path) })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setBusy(false))
      }

      const startRename = (name) => { setRenameTarget(name); setNewName(name) }
      const doRename = () => {
        const n = newName.trim()
        if (!n || !renameTarget || n === renameTarget) { setRenameTarget(null); return }
        setBusy(true)
        api('POST', '/dsh-remote/fs', { action: 'rename', from: join(path, renameTarget), to: join(path, n) })
          .then(() => { setMsg('已重命名'); setRenameTarget(null); loadDir(path) })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setBusy(false))
      }
      ...
      const crumbs = () => {
        ...
        const list = [{ name: '/', p: '/' }]
        let acc = ''
        for (const seg of parts) { acc += '/' + seg; list.push({ name: seg, p: acc }) }
        return list
      }

      const row = (it, i) => {
        const isSel = sel === it.name
        const style = isSel ? fmStyles.rowHover : fmStyles.row
        const icon = it.dir ? '📁' : (it.link ? '🔗' : '📄')
        return React.createElement('div', {
          key: it.name + i,
          style,
          onClick: () => setSel(isSel ? null : it.name),
          onDoubleClick: () => { if (it.dir) enter(it.name); else download(it.name) },
        },
          React.createElement('span', { style: fmStyles.icon }, icon),
          renameTarget === it.name
            ? React.createElement('input', { value: newName, onChange: (e) => setNewName(e.target.value), onKeyDown: (e) => { if (e.key === 'Enter') doRename(); if (e.key === 'Escape') setRenameTarget(null) }, autoFocus: true, style: { ...inputS, flex: 1, fontFamily: 'monospace' } })
            : React.createElement('span', { style: fmStyles.name }, it.name),
          React.createElement('span', { style: fmStyles.size }, it.dir ? '' : fmtSize(it.size)),
          React.createElement('span', { style: fmStyles.mtime }, fmtTime(it.mtime)),
        )
      }

      return React.createElement('div', { style: fmStyles.overlay, onClick: (e) => { if (e.target === e.currentTarget) onClose() } },
        React.createElement('div', { style: { ...fmStyles.panel, ...win.style, height: win.minimized ? 'auto' : win.style.height }, onClick: (e) => e.stopPropagation() },
          React.createElement('div', { style: { ...fmStyles.head, cursor: 'move', userSelect: 'none' }, onPointerDown: win.onPointerDown },
            React.createElement('div', { style: { fontSize: 14, fontWeight: 600, flex: 1 } }, '远程文件（dsh-remote）'),
            React.createElement('span', { style: { fontSize: 11, opacity: 0.7, fontFamily: 'monospace' } }, info ? (info.username + '@' + info.host) : ''),
            React.createElement('button', { style: buttonS, onClick: () => loadDir(path), title: '刷新' }, '↻'),
            React.createElement('button', { style: buttonS, onClick: win.toggleMinimize, title: win.minimized ? '还原' : '最小化' }, win.minimized ? '▢ 还原' : '— 最小化'),
            React.createElement('button', { style: { ...buttonS, fontWeight: 600 }, onClick: onClose }, '关闭 ✕'),
          ),
          win.minimized
            ? null
            : React.createElement(React.Fragment, null,
                React.createElement('div', { style: fmStyles.pathBar },
                  crumbs().map((c, i) => React.createElement('span', { key: i },
                    React.createElement('a', { href: '#', onClick: (e) => { e.preventDefault(); loadDir(c.p) }, style: { color: T.ok, fontSize: 12.5, fontFamily: 'monospace' } }, c.name),
                    i < crumbs().length - 1 ? React.createElement('span', { style: { opacity: 0.4, margin: '0 2px' } }, '/') : null,
                  )),
                ),
                React.createElement('div', { style: { display: 'flex', gap: 6, padding: '8px 14px', borderBottom: '1px solid ' + T.border, flexShrink: 0, alignItems: 'center', flexWrap: 'wrap' } },
                  React.createElement('button', { style: buttonS, onClick: () => loadDir(parent(path)), title: '上一级' }, '⬆ 上级'),
                  React.createElement('button', { style: buttonS, onClick: () => fileInputRef.current && fileInputRef.current.click(), disabled: busy }, '⬆ 上传文件'),
                  React.createElement('button', { style: buttonS, onClick: () => sel && download(sel), disabled: !sel || busy }, '⬇ 下载'),
                  React.createElement('button', { style: buttonS, onClick: () => setMkdirOpen((v) => !v), disabled: busy }, '📁 新建目录'),
                  React.createElement('button', { style: buttonS, onClick: () => sel && startRename(sel), disabled: !sel || busy }, '✏ 重命名'),
                  React.createElement('button', { style: { ...buttonS, color: T.danger }, onClick: () => sel && doDelete(sel), disabled: !sel || busy }, '🗑 删除'),
                  msg ? React.createElement('span', { style: { fontSize: 12, color: T.ok, marginLeft: 'auto' } }, msg) : null,
                  React.createElement('input', { ref: fileInputRef, type: 'file', style: { display: 'none' }, onChange: (e) => { const f = e.target.files && e.target.files[0]; if (f) upload(f); e.target.value = '' } }),
                ),
                mkdirOpen
                  ? React.createElement('div', { style: { display: 'flex', gap: 6, padding: '6px 14px', borderBottom: '1px solid ' + T.border, alignItems: 'center' } },
                      React.createElement('span', { style: { fontSize: 12, opacity: 0.8 } }, '目录名:'),
                      React.createElement('input', { value: mkdirName, onChange: (e) => setMkdirName(e.target.value), onKeyDown: (e) => { if (e.key === 'Enter') doMkdir(); if (e.key === 'Escape') setMkdirOpen(false) }, placeholder: 'newdir', style: { ...inputS, fontFamily: 'monospace' } }),
                      React.createElement('button', { style: buttonS, onClick: doMkdir, disabled: busy || !mkdirName.trim() }, '创建'),
                    )
                  : null,
                err ? React.createElement('div', { style: { padding: '6px 14px', color: T.danger, fontSize: 12 } }, err) : null,
                React.createElement('div', { style: fmStyles.table },
                  loading
                    ? React.createElement('div', { style: { padding: 16, opacity: 0.7 } }, '加载中…')
                    : (items.length
                        ? items.map(row)
                        : React.createElement('div', { style: { padding: 16, opacity: 0.6 } }, '（空目录）')),
                ),
                React.createElement('div', { style: { padding: '6px 14px', borderTop: '1px solid ' + T.border, fontSize: 11, opacity: 0.65, flexShrink: 0 } }, '提示：双击目录进入，双击文件下载；单击选中后可 下载/重命名/删除。上传会把本地文件传到当前目录。'),
                React.createElement('div', {
                  onPointerDown: win.onResizeDown,
                  style: { position: 'absolute', right: 0, bottom: 0, width: 18, height: 18, cursor: 'nwse-resize', zIndex: 5 },
                  title: '拖动调整大小',
    ...
    function FileManagerEntry(props) {
      const [open, setOpen] = React.useState(false)
      const wide = !!(props && props.wide)
      return React.createElement(React.Fragment, null,
        React.createElement('button', {
          ...
          wide ? React.createElement('span', null, '远程文件') : null,
        ),
        open ? React.createElement(FileManager, { onClose: () => setOpen(false) }) : null,
      )
    }
    ...
    function apply(ctx) {
      ...
      slots.inject(
        ...
        () => slots.inject('sidebar.workspaces.directoryFlow',
          function* () {
            // Both directory-flow holes are SINGLE slots: the slot service rejects a
            // second registration at the SAME priority, which makes whichever plugin
            // registers second fail to load entirely. @dsh-external/dsh-webui also
            // fills both holes at -100, so register at -110: a lower priority shadows
            // it (lowest renders), keeping this plugin's 本机/远程 picker on screen.
            // Use -90 instead if dsh-webui's picker should win.
            yield slots.register({ name: 'conversation.hero.workspace.directoryFlow', id: 'dsh-remote', priority: -110 }, DirPicker)
            yield slots.register({ name: 'sidebar.workspaces.directoryFlow', id: 'dsh-remote', priority: -110 }, DirPicker)
          },
        ),
      )
      slots.inject('sidebar.footer.action', () =>
        slots.register({ name: 'sidebar.footer.action', id: 'dsh-remote-terminal', order: 50 }, RemoteTerminalEntry),
      )
      slots.inject('sidebar.footer.action', () =>
        slots.register({ name: 'sidebar.footer.action', id: 'dsh-remote-files', order: 51 }, FileManagerEntry),
      )
    }
